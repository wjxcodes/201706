...
(function($) {
	...
	$.alerts = {
		...
		_show: function(title, msg, value, type, callback) {
			...
			$("#popup_container").addClass('container'+type);
			...
			$("#popup_title").html($("#popup_title").html()+title);
			...
			switch( type ) {
				...
				break;
				case 'info':
					$("#popup_message").html('<img style="vertical-align:middle;" src="/Public/plugin/alert/images/loading.gif"/>'+$("#popup_message").html());
					$("#popup_message").css({'padding-left':'0px'});
					$('#popup_container').css({'max-width':'300px'});
				break;
				case 'frame':
					$('#popup_title').html('<span id="close">X</span>'+$('#popup_title').html());
					$("#close").click( function() {
						$.alerts._hide();
						if( callback ) callback(true);
					});
					//$("#close").focus().keypress( function(e) {
					//	if( e.keyCode == 13 || e.keyCode == 27 ) $("#popup_ok").trigger('click');
					//});
					$("#popup_message").css({'height':$(window).height()-80,'overflow':'auto'});
					...
					$("#popup_container").css({
						top: ($(window).height()-$("#popup_container").outerHeight())/2-5
					});
				break;
				case 'confirm':
		...
		_hide: function() {
			if($("#popup_container").length>0) $("#popup_container").remove();
