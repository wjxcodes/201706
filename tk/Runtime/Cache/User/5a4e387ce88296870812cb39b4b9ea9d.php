...
<div class="operate">
    ...
    <!-- 查询区域 -->
    <form method="POST" action="/User/UserCollect">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value="<?php echo ($_REQUEST['name']); ?>" title="用户查询" class="medium" ></span></div>
    ...
    <div id="searchM" class=" none search cBoth">
        <table border="0" cellpadding="1" cellspacing="3" width="100%">
            <tr>
                ...
                <td class="tRight" width="80">来源：</td>
                <td>
                    <select name="From">
                        <option value="0">全部</option>
                        <option value="1">组卷</option>
                        <option value="2">提分</option>
                    </select>
                </td>
            </tr>
        </table>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["CollectID"]); ?>"></td>
        <td><?php echo ($node["CollectID"]); ?></td>
        <td><?php echo ($node["FavName"]); ?></td>
        <td><?php echo ($node["ThisName"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["LoadTime"])); ?></td>
        <td><?php echo ($node["UserName"]); ?></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><a href="#" class="btdelete" thisid="<?php echo ($node["CollectID"]); ?>">删除</a>&nbsp;</td>
