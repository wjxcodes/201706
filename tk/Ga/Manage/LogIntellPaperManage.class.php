 ...
 */
/**
 * 文档控制器类，用于处理只能组卷操作记录相关操作
 */
namespace  Ga\Manage;
class LogIntellPaperManage extends BaseController  {
    /**
     * @智能组卷列表页
     * @author demo
     */
    public function index(){
        ...
        $logIntellPaper=$this->getModel('LogIntellPaper');
        $count = $logIntellPaper->selectCount(
            $where['data'],
            "PaperID"); // 查询满足要求的总记录数
    ...
    public function showMsg(){
        ...
        $logIntellPaper=$this->getModel('LogIntellPaper');
        $thisTpl=$logIntellPaper->selectData(
            '*',
            $where,
            '',
            '1');
    ...
    public function export() {
        ...
        $logData = $logIntellPaper->pageData(
            '*',
            $where['data'],
            'PaperID desc',
            $page);
        ...
        $keyName=array('编号','用户名','所含试题','时间','所属学科');
        $keyWidth=array('10','20','20','50','20','20');
        $excelName=array('title'=>'智能组卷记录日志列表','excelName'=>'智能组卷记录信息导出Excel');
