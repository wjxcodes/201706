...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('Doc/DocType/updateCache');?>">更新缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($typeArray)): $i = 0; $__LIST__ = $typeArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["TypeID"]); ?>"></td>
        <td><?php echo ($node["TypeID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TypeID"]); ?>"><?php echo ($node["TypeName"]); ?></a></td>
        <td><?php echo ($node["Tag"]); ?></td>
        <td><?php echo ($node["GradeName"]); ?></td>
        <td><?php if($node["DefaultTest"] == '1'): ?>测试<?php else: ?><font color="#f00">不测试</font><?php endif; ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><?php echo ($node["ChapterOrder"]); ?></td>
        <td><?php if($node['IfHidden'] == 0): ?>不隐藏<?php else: ?>隐藏<?php endif; ?></td>
        <td><?php if($node['LimitDown'] == 1): ?>提供下载<?php else: ?><font color="#f00">不提供下载</font><?php endif; ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TypeID"]); ?>">编辑</a>&nbsp;</td>
