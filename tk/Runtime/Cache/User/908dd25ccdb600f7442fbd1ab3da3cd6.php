...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('UserLevel/updateCache');?>">更新缓存</a> <a href="<?php echo U('UserLevel/showLevelLucre');?>">查看等级权限对照表</a></div>
<!--  功能操作区域  -->
<div class="operate">
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($typeArray)): $i = 0; $__LIST__ = $typeArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["LevelID"]); ?>"></td>
        <td><?php echo ($node["LevelID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["LevelID"]); ?>"><?php echo ($node["LevelName"]); ?></a></td>
        <td><?php echo ($node["LevelExpMin"]); ?></td>
        <td><?php echo ($node["LevelExpMax"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["LevelID"]); ?>">编辑</a>&nbsp;
            <a href="<?php echo U('UserLevel/addUserLevelValue',array('id'=>$node[LevelID]));?>">添加等级权限</a>
            <a href="<?php echo U('UserLevel/showThisLevelMsg',array('id'=>$node[LevelID]));?>">该等级对应权限</a>
        </td>
