...
DateInput.prototype = {
  ...
  insideSelector: function(event) {
    ...
    if($('.dialog').length>0){
	    offset.left += $('.dialog').position().left;
	    offset.right += $('.dialog').position().left;
	    offset.top += $('.dialog').position().top;
	    offset.bottom += $('.dialog').position().top;
    }
  ...
  setPosition: function() {
    ...
    if($('.dialog').length>0){
    	this.rootLayers.css({
	      top: offset.top + this.input.outerHeight()-$('.dialog').offset().top,
	      left: offset.left-$('.dialog').offset().left
	    });
	}
