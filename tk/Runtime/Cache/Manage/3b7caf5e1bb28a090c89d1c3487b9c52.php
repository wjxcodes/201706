...
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Manage/KlStudy">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value="<?php echo ($_REQUEST['name']); ?>" title="内容查询" class="medium" ></span></div>
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            ...
            <TD><INPUT TYPE="text" NAME="id" class="small" value="<?php echo ($_REQUEST['id']); ?>" ></TD>
            <TD class="tRight" width="60">内容：</TD>
            <TD><INPUT TYPE="text" NAME="Content" class="small" value="<?php echo ($_REQUEST['Content']); ?>" ></TD>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["StudyID"]); ?>"></td>
        <td><?php echo ($node["StudyID"]); ?></td>
        <td><?php echo ($node["KlName"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["LoadTime"])); ?></td>
        <td><?php if(($node["Status"]) == "0"): ?>正常<?php else: ?><font color="red">锁定</font><?php endif; ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["StudyID"]); ?>">编辑</a>&nbsp;</td>
...
<script>
$('#subject').subjectSelectChange('/Manage/KlStudy',{'style':'getMoreData','list':'knowledge'});
$('.selectKnowledge').knowledgeSelectChange('/Manage/KlStudy');
</script>
<!-- 主页面结束 -->
