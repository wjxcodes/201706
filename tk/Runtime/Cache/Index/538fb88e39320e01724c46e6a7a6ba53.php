...
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Index/MyTag">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value="<?php echo ($_REQUEST['name']); ?>" title="标签名称查询" class="medium" ></span></div>
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            <TD class="tRight" width="80">标签名称：</TD>
            <TD><INPUT TYPE="text" NAME="Title" class="small" value="<?php echo ($_REQUEST['Title']); ?>" ></TD>
            <TD class="tRight" width="80">标签标识：</TD>
            <TD ><INPUT TYPE="text" NAME="TagName" class="small" value="<?php echo ($_REQUEST['TagName']); ?>"></TD>
            <TD class="tRight" width="80">所属分类：</TD>
            <TD><SELECT class="medium bLeft" NAME="Type">
            <option value="">选择</option>
            <?php if(is_array($typeListArray)): $i = 0; $__LIST__ = $typeListArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><option value="<?php echo ($node["Type"]); ?>" <?php if(($_REQUEST['Type']) == $node["Type"]): ?>selected="selected"<?php endif; ?>><?php echo ($node["Type"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </SELECT></TD>
        </TR>
        </TABLE>
    </div>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["TagID"]); ?>"></td>
        <td><?php echo ($node["TagID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TagID"]); ?>"><?php echo ($node["Title"]); ?></a></td>
        <td><?php echo ($node["Description"]); ?></td>
        <td>{#wln:tag tagname="<?php echo ($node["TagName"]); ?>"#}</td>
        <td><?php echo ($node["Type"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["LoadTime"])); ?></td>
        <td><?php echo ($node["Admin"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TagID"]); ?>">编辑</a>&nbsp;</td>
