...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('Types/updateCache');?>">更新缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Manage/Types">
    <div class="fRig">
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["TypesID"]); ?>"></td>
        <td><?php echo ($node["TypesID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TypesID"]); ?>"><?php echo ($node["TypesName"]); ?></a></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php if($node["Volume"] == 1): ?>分卷Ⅰ<?php else: ?>分卷Ⅱ<?php endif; ?></td>
        <td><?php echo ($node["Num"]); ?></td>
        <td><?php if($node["TypesScore"] == 1): ?>按小题计分<?php endif; if($node["TypesScore"] == 2): ?>按大题计分<?php endif; ?>|
            <?php if($node["IfPoint"] == 1): ?>可设置0.5分<?php else: ?>不可设置0.5分<?php endif; ?>|
            默认分值为<?php echo ($node["DScore"]); ?>分|
            最大分值为<?php echo ($node["MaxScore"]); ?>分|
            试题任务加<?php echo ($node["ScoreNormal"]); ?>分|
            入库试题加<?php echo ($node["ScoreIntro"]); ?>分|
            放弃标引扣<?php echo ($node["ScoreMiss"]); ?>分|
            图片版加<?php echo ($node["ScorePic"]); ?>分
        </td>
        <td><?php if($node["TypesStyle"] == '3'): ?>非选择题<?php elseif($node["TypesStyle"] == '2'): ?>选择非选择混合<?php else: ?>选择题<?php endif; ?>|
        <?php if($node["IfChooseType"] == 0): ?>可选择题型类别<?php else: ?>不可选择题型类别<?php endif; ?></td>
        <td><?php if($node["IfSingle"] == 1): ?>非单选题<?php else: ?>单选题<?php endif; ?>|
            <?php if($node["IfSearch"] == 1): ?>可搜索试题<?php else: ?>不可搜索试题<?php endif; ?>|
            <?php if($node["IfChooseNum"] == 1): ?>不带小题<?php else: ?>带小题<?php endif; ?>|
            <?php if($node["IfDo"] == 1): ?>无选做题<?php else: ?>有选做题<?php endif; ?>
            <?php if($node["SelectType"] == 1): ?>不忽略小题<?php else: ?>忽略小题<?php endif; ?></td>
        <td><?php if($node["Underline"] == 1): ?>下划线<?php endif; ?>
        <?php if($node["CardIfGetTest"] == 1): ?>显示题文<?php endif; ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TypesID"]); ?>">编辑</a>&nbsp;</td>
