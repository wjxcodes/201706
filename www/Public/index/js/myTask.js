$.MyTask = {
    ...
    ajaxGetPaper : function(page) {
        ...
        $.post(U('UserInfo/myTasksList'),data,function(data){
    ...
    tableList : function(paperdata) {
        ...
        "<th align=\"left\" width=\"300\">任务名称</th>" +
        "<th align=\"left\" width=\"100\">任务状态</th>" +
        "<th align=\"left\" width=\"80\">任务等级</th>" +
        ...
        if(paperdata) {
            ...
            for (var i = 0, len = paperdata.length; i < len; i++) {
                ...
                if(1 == status){
                    if(paper.Url.indexOf('/Home') != 0){
                        if(paper.Url.indexOf('/Teacher') === 0){
                            paper.Url = paper.Url.replace('/','');
                            paper.Url = '/Teacher-Index-index-u-_'+paper.Url.replace('-','_');
                        }
                        goWork += '&nbsp;&nbsp;[<a href="'+paper.Url+'" target="_blank">现在做任务</a>]';
                    }else{
                        goWork += '&nbsp;&nbsp;[<a href="'+paper.Url+'">现在做任务</a>]';
                    }
                }
                var info = '';
                switch(status){
                ...
                html.push("<tr><td width=\"30\" align=\"center\">" + id +
                        "</td><td class=\"paperName\"><a href=\"" + U('MissionHall/taskDetails?id='+ mhtid) + "\">" + title + "</a>" + goWork +
                        "</td><td class=\"paperName\">" + info + 
                        "</td><td class=\"paperName\">" + levelArr[level] + 
