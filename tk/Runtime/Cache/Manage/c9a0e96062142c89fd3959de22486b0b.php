...
<div class="operate">
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            <TD class="tRight" width="60">管理员：</TD>
            <TD><INPUT TYPE="text" NAME="AdminName" class="small" value="<?php echo ($_REQUEST['AdminName']); ?>" ></TD>
            ...
            <TD><SELECT class="medium bLeft" NAME="GroupID">
            <option value="">选择</option>
            <?php if(is_array($adminGroupArray)): $i = 0; $__LIST__ = $adminGroupArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><option value="<?php echo ($node["PUID"]); ?>" <?php if(($_REQUEST['GroupID']) == $node["PUID"]): ?>selected="selected"<?php endif; ?>><?php echo ($node["AdminGroup"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </SELECT></TD>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["AdminID"]); ?>"></td>
        <td><?php echo ($node["AdminID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["AdminID"]); ?>"><?php echo ($node["AdminName"]); ?></a></td>
        <td><?php echo ($node["RealName"]); ?></td>
        <td><?php echo ($node["Email"]); ?></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php echo ($node["AdminGroup"]); ?></td>
        <td><?php if(($node["Status"]) == "0"): ?>正常<?php else: ?><font color="red">锁定</font><?php endif; ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["AdminID"]); ?>">编辑</a>&nbsp;</td>
