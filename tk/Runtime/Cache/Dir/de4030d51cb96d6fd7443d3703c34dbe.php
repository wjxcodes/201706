...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> | <a href="<?php echo U('DirExamType/index');?>" title="考试类型管理" tabindex="-1">考试类型管理</a></div>
<!--  功能操作区域  -->
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Dir/DirTemplate">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value="<?php echo ($_REQUEST['name']); ?>" title="模板查询" class="medium" ></span></div>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["TempID"]); ?>"></td>
        <td><?php echo ($node["TempID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TempID"]); ?>"><?php echo ($node["TempName"]); ?></a></td>
        <td><?php echo ($node["TypeName"]); ?></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php if(($node["IfDefault"]) == "0"): ?><font color="red">是</font><?php else: ?><span>否</span><?php endif; ?></td>
        <td><?php echo ($node["UserName"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["AddTime"])); ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["TempID"]); ?>">编辑</a>&nbsp;</td>
