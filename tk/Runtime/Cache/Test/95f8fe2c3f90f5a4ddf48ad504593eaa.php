...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('TestMark/updateCache');?>">生成缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["MarkID"]); ?>"></td>
        <td><?php echo ($node["MarkID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["MarkID"]); ?>"><?php echo ($node["MarkName"]); ?></a></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php echo ($node["MarkList"]); ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["MarkID"]); ?>">编辑</a>&nbsp;</td><?php endforeach; endif; else: echo "" ;endif; ?>
