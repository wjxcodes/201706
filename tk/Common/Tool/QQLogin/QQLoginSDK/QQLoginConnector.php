 ...
 */
/*
 * @brief QQLogin Api外部对象，调用接口全部依赖于此对象
 **/
class QQLoginConnector extends QQLoginTool{
    ...
    private function _applyAPI($arr, $argsList, $baseUrl, $method){
            ...
            if(!isset($arr[$tmpKey]) || $arr[$tmpKey] === ""){
                ...
                if($tmpVal == $pre){//则使用默认的值
                    ...
                    if($v = $_FILES[$tmpKey]){
                        ...
                        return $this->showError("api调用参数错误","未传入参数$tmpKey");
                    }
                }
            }
        ...
        foreach($optionArgList as $val){
            ...
            if(! $n){
                $str = implode(",",$val);
                return $this->showError("api调用参数错误",$str."必填一个");
            }
        }
    ...
    public function __call($name,$arg){
        ...
        if(empty($this->APIMap[$name])){
            return $this->showError("api调用名称错误","不存在的API: <span style='color:red;'>$name</span>");
        }
        ...
        $result=$this->_applyAPI($arg[0], $argsList, $baseUrl, $method);
        ...
        if($name != "get_tenpay_addr"){
            ...
            $responseArr = $this->simple_json_parser($result);
        }
        ...
        if($responseArr['ret'] == 0){
            ...
            return $this->showError($response->ret, $response->msg);
        }
