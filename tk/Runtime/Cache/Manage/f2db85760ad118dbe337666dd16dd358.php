...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('Knowledge/updateCache');?>">更新知识点缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Manage/Knowledge">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value='<?php echo ($_REQUEST['name']); ?>' title="知识点查询" class="medium" ></span></div>
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            ...
            <TD><INPUT TYPE="text" NAME="KlName" class="small" value='<?php echo ($_REQUEST['KlName']); ?>' ></TD>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["KlID"]); ?>"></td>
        <td><?php echo ($node["KlID"]); ?></td>
        <td><?php echo ($node["KlChild"]); ?> <a href="<?php echo U('Knowledge/index',array('knowledge'=>$node['KlID']));?>"><?php echo ($node["KlName"]); ?></a></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php echo ($node["Frequency"]); ?></td>
        <td><?php if( $node["IfTest"] == 1 ): ?>是<?php else: ?>否<?php endif; ?></td>
        <td><?php if( $node["IfInChoose"] == 0 ): ?>适用全部<?php else: ?>适用选做题<?php endif; ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["KlID"]); ?>">编辑</a>&nbsp;</td>
...
<script language="javascript">
    $('#subject').subjectSelectChange('/Manage/Knowledge',{'style':'getMoreData','list':'knowledge'});
    $('.selectKnowledge').knowledgeSelectChange("/Manage/Knowledge");
</script>
<!-- 主页面结束 -->
