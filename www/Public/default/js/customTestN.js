$.CustomTestStore = { 
    ...
    //相关数据显示对话框
    showDialog : function(elemnt,title,delay,msg,type,YesHandler,NoHandler){
        var btn = [];
        btn.push('确定');
        if(type == 2){
            btn.push('取消');
        ...
        layer.open({
            ...
            yes : YesHandler || function(){ layer.closeAll(); },
            cancel : NoHandler || function(){ layer.closeAll(); }
        });
    },
        ...
        if(!ifImage){
            ...
            $('#format').click(function(){
                ...
                if(FormatTextManager.formatContent()){
                    $.CustomTestStore.showDialog('msgdiv','错误提示',500,FormatTextManager.err,1);
                    return false;
                }
            ...
            $('#save').click(function(){
                if($.Editor.requestMark){
                    $.CustomTestStore.showDialog('msgdiv','提示',500,'内容已提交，正在处理中，请稍候！',1);
                    return false;
                }
                ...
                if(data){
                    ...
                    $.post(U($.CustomTestStore.url+'/save'),data,function(result){
                        ...
                        // }
                        var msg = result['data'];
                        msg += '<br>点击 <strong>确定</strong> 再添一题，点击 <strong>取消</strong> 跳转校本试题！</a>';
                        $.CustomTestStore.showDialog('msgdiv','提示',500,msg, 2, function(){
                            window.location.href = U('Custom/CustomTestStoreN/add');
                        },
                        function(){
                            window.location.href = U('Custom/CustomTestStoreN/index');
                        });
                    });
                }
                return false;
    ...
    showTip:function(message){
        ...
        $(document).on('click','#goIndexOriginality',function(){
            top.window.location.href=U('Index/Special/orginality?SubjectID='+Cookie.Get('SubjectId'));
            Cookie.Del('user_ORIGINALITY');
        });
        $(document).on('click','#quitOriginality',function(){
            window.location.href=U('Home/Index/content');
            Cookie.Del('user_ORIGINALITY');
        });
    },
    //编辑内容
    ...
    validate : function(){
        ...
        if(!$.CustomTestStore.username){
            $.CustomTestStore.showDialog('msgdiv','错误提示',500,'用户名不能为空',1);
            return false;
        }
        ...
        var type = $('#types').val();
        if(!type){
            type = types[$.CustomTestStore.subjectID][0]['TypesID'];
        ...
        if(errors.length > 0){
            ...
            $.CustomTestStore.showDialog('msgdiv','错误提示',500,error.join('，'),1);
...
$.ImageTest = {
    ...
    syncImage:function(no){
        var self = this;
        var post = function(){
            $.post(U('Custom/CustomTestStore/webImagePoll'+'?n='+no),function(e){
                if(e.status==1){
                    $('.submitBtn').show();
                    //传给函数，用于显示新图片dom
    ...
    getQrCode:function(username){
        ...
        $('.qrCode').attr('src', U('Custom/CustomTestStoreN/qrCode') + '?n=' + no + '&u=' + username).show();
        setTimeout(function () {
            self.syncImage(no);
        }, 5000);
    },
    ...
    avatarUpload:function() {
        ...
        fileUpload.uploadify({
            ...
            'uploader': U('Custom/CustomTestStoreN/avatarUpload?username=' + username + '&key=' + key),
            'method': 'post',
            'multi': false,
    ...
    addImgList:function(data){
        ...
        var imgTestHtml = '<li class="clearfix imgTest" id="imgTest'+length+'">' +
            ...
            '            <img height="200" width="300"  src="'+data.picPath+'" alt="已上传图片" id="prevImg"/>' +
            '        </div>'+
            '        <div class="img-intro">'+
    ...
    avatarSave:function(num) {
        ...
        $.post(U('Custom/CustomTestStoreN/imgCutSave'), {picPath:thisPicPath, width: width, height: height, x1: x1, y1: y1}, function (e) {
    ...
    bindImg:function(){
        ...
        $('#saveImgTest').on('click',function(){
            ...
            if(data.length==0){
                $.CustomTestStore.showDialog('msgdiv','提示',500,'<div>请先上传图片编辑！</div>',1);
                return false;
            }
            $.post(U('Custom/CustomTestStoreN/imgTestSave'),{'data':data,'attr':self.getTestAttr()},function(e){
                if(e.status == 1){
                    if($.myCommon.backLogin(e)==false){
                        return false;
                    }
                    ...
                    msg += '<br>点击 <strong>确定</strong> 再添一题，点击 <strong>取消</strong> 跳转校本试题！</a>'
                    $.CustomTestStore.showDialog('msgdiv','提示',500,msg, 2, function(){
                        window.location.href = U('Custom/CustomTestStoreN/photograph');
                    },
                    function(){
                        window.location.href = U('Custom/CustomTestStoreN/index');
                    });
                    ...
                    $.CustomTestStore.showDialog('msgdiv','提示',500,alertHtml,1);
                }
            })
            return false;
        });
    ...
    pageInit:function(data,originality,url,ifImage, act){
        ...
        if(!originality['ttID'] && 'add' == act){
            ...
            if(originality['ttID']){
                ...
                if(originality['klID']){
                    $.post(U('Custom/CustomTestStoreN/getData'),params,function(result){
        ...
        $('.knowledge-select-change-container').SelectChangeHandle({
            ...
            vv : 'KlName',
            identify : 'knowledge',
            loadArea : '.knowledge-select-change-container-inputs'
        });
        $('.chapter-select-change-container').SelectChangeHandle({
            ...
            identify : 'chapter',
            allowEmptyVal : true,
            loadArea : '.chapter-select-change-container-inputs'
        });
    }
};
    ...
    return $(this).each(function(){
        ...
        that.find(opts.loadBtn).click(function(){
            ...
            if(inputIsExist(vals)){
                var div = $('<div></div>');
                $(opts.loadArea).append(div.SelectText({
                    _text : text.join('&nbsp;&gt;&nbsp;'),
                    _name : opts.kv,
                    val : vals
                }));
...
$.fn.SelectText = function(opts){
    ...
    function getText(){
        ...
        return '<li><p class="">'+opts._text+'<i class="iconfont del-btn input-del" title="删除">&#xe604;</i><input type="hidden" class="'+opts._name+'_inputs" value="'+opts.val+'"></p></li>';
    }
}