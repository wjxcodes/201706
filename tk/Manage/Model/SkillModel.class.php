...
class SkillModel extends BaseModel{
    ...
    /**
     * 获取多级列表树形结构
     * @param array $children 根据技能id为索引的下一级数据集
     * @param array $klID 技能id
     * @return mabo
     */
    ...
    public function setCache(){
        ...
        //获取以技能id为索引的数据，除基础数据外获取是否是最后一级；获取当前数据的下一级；获取当前数据的父类路径
        $knowledgeArr = array();
        foreach($buffer as $iBuffer){
        ...
        S($this->primary,$knowledgeArr);//所有技能缓存 以id为索引
        S($this->primary.'Parent',$parent);//所有技能对应父类路径数据集缓存
        S($this->primary.'Next',$next);//所有技能对应下一级数据集缓存
        S($this->primary.'BySubject3',$subject);//根据学科区分技能
    }
     ...
     * @param index parent 技能父类缓存
     * @param index self 技能当前缓存
     * @param index ID 需要显示的技能id 支持数组和逗号间隔字符串
