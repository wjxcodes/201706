...
$(document).ready(function(){
    $('#subject').subjectSelectChange('/Index/WlnSpecial',{'style':'getMoreData','list':'special'});
});
</script>
<!-- 主页面开始 -->
...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('Index/WlnSpecial/updateCache');?>">更新缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM METHOD="POST" ACTION="/Index/WlnSpecial">
    <div class="fRig">
        <div class="fLeft"><span id="key"><INPUT id="name" TYPE="text" NAME="name" value="<?php echo ($_REQUEST['name']); ?>" title="专题名称查询" class="medium" ></span></div>
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            <TD class="tRight" width="60">专题：</TD>
            <TD><INPUT TYPE="text" NAME="SpecialName" class="small" value="<?php echo ($_REQUEST['SpecialName']); ?>" ></TD>
            ...
            <TD><SELECT  class="medium bLeft" NAME="PID"  id="special">
            <option value="">选择</option>
            </SELECT></TD>
        </TR>
        </TABLE>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["SpecialID"]); ?>"></td>
        <td><?php echo ($node["SpecialID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["SpecialID"]); ?>"><?php echo ($node["SpecialName"]); ?></a></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php echo ($node["OrderID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["SpecialID"]); ?>">编辑</a>&nbsp;</td>
