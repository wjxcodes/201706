...
var data=$.CustomTestStore.validate();if(data){$.Editor.requestMark=true;console.log(data);$.post(U($.CustomTestStore.url+'/save'),data,function(result){$.Editor.requestMark=false;if($.myCommon.backLogin(result)==false){return false;}
...
var container=$('.chapter-select-change-container').find('.inputs');var chatper=data['chapter'][val];container.append($('<div></div>').SelectText({_text:chatper['ChapterName'],val:chatper['ChapterID'],_name:'ChapterID'}));}
for(var val in data['skill']){if(!data['skill'][val]['SkillID']){continue;}
var container=$('.skill-select-change-container').find('.inputs');var skill=data['skill'][val];container.append($('<div></div>').SelectText({_text:skill['SkillName'],val:skill['SkillID'],_name:'SkillID'}));}
for(var val in data['capacity']){if(!data['capacity'][val]['CapacityID']){continue;}
var container=$('.capacity-select-change-container').find('.inputs');var capacity=data['capacity'][val];container.append($('<div></div>').SelectText({_text:capacity['CapacityName'],val:capacity['CapacityID'],_name:'CapacityID'}));}},InitDivHeight:function(){var height=$(window).height()-50;$("#divbox").css({'height':height-5,'overflow-y':'auto','overflow-x':'hidden'});},setBasicData:function(){var params={};if(this.subjectID){$('#types').html('<option value="">加载中请稍候...</option>');$('#knowledge').html('<option value="">加载中请稍候...</option>');$('#special').html('<option value="">加载中请稍候...</option>');$('#chapter').html('<option value="">加载中请稍候...</option>');params['subjectID']=this.subjectID;params['list']=['knowledge','chapter','special','types','grade','skill','capacity'];params['style']='getMoreData';params=this.formatParams(params);$.post(U($.CustomTestStore.url+'/getData'),params,function(result){if($.myCommon.backLogin(result)==false){return false;};var datas=result['data'];$('#types').html('<option value="">请选择题型</option>'+$.CustomTestStore.addData(datas['types'],{val:'TypesID',text:'TypesName',isSelected:true,selectedById:$.CustomTestStore.types,attributes:{testnum:1,TypesStyle:'%s',IfChooseType:'%s'}}));$('#grade').html('<option value="">请选择年级</option>'+$.CustomTestStore.addData(datas['grade'],{val:'GradeID',text:'GradeName',isSelected:true,selectedById:$.CustomTestStore.grade}));$('#knowledge').html('<option value="">请选择主干知识点</option>'+$.CustomTestStore.addData(datas['knowledge'],{val:'KlID',text:'KlName'}));$('#chapter').html('<option value="">请选章节，避免超纲</option>'+$.CustomTestStore.addData(datas['chapter'],{val:'ChapterID',text:'ChapterName'}));$('#skill').html('<option value="">请选技能</option>'+$.CustomTestStore.addData(datas['skill'],{val:'SkillID',text:'SkillName'}));$('#capacity').html('<option value="">请选能力</option>'+$.CustomTestStore.addData(datas['capacity'],{val:'CapacityID',text:'CapacityName'}));},'json');}},formatParams:function(params){var param=[];for(var p in params){var str='';if(p==='list'){str='list='+params[p].join(',');}else{str=p+'='+params[p];}
param.push(str);}
...
data['SkillID']=[];var skill=$(".SkillID_inputs");skill.each(function(){data['SkillID'].push($(this).val());});if(data['SkillID'].length==0){}
data['CapacityID']=[];var capacity=$(".CapacityID_inputs");capacity.each(function(){data['CapacityID'].push($(this).val());});if(data['CapacityID'].length==0){}
...
$('.knowledge-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'KlID',vv:'KlName',identify:'knowledge'});$('.chapter-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'ChapterID',vv:'ChapterName',identify:'chapter',allowEmptyVal:true});$('.skill-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'SkillID',vv:'SkillName',identify:'skill',});$('.capacity-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'CapacityID',vv:'CapacityName',identify:'capacity',});}};$.fn.SelectChangeHandle=function(option){return $(this).each(function(){var opts=$.extend({loadBtn:'.append',classname:'select',loadArea:'.inputs',subjectid:false,allowEmptyVal:false,kv:'',vv:'',identify:'',url:'',callback:function(result,currentSel,selects){if($.myCommon.backLogin(result)==false){return false;};var datas=result['data'];finished=true;if(!datas||datas['length']=='0'){return false;}
