...
class CustomTestStoreController extends DefaultController{
    ...
    public function save(){
        ...
        $this->setBack('保存成功，试题正在处理，一分钟后将会显示在校本试题中！');
    }
    ...
    public function edit(){
        ...
        $this->display('CustomTestStore/add');
    }
    ...
    public function mobile(){
        ...
        if(IS_AJAX){
            ...
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64, $result)){
                ...
                if (file_put_contents(realpath('./').'/Uploads/customTest'.$filename, base64_decode(str_replace($result[1], '', $base64)))){
                    $this->setBack('success');
                }else{
    ...
    public function docList(){
        ...
        if($list > $prepage){
            ...
            $url = '/Home/CustomTestStore/docList/p/%s/status/%s';
            $params = array($p, $status);
            if($title){
                $url .= '/title/%s';
                $params[] = formatString('encodeUrl',$title);
            }
            array_unshift($params, $url);
            $pagtion['p'] = call_user_func_array('sprintf', $params);
            $count = ceil($list[0] / $prepage);
            if($page != $p){
                $p += 2;
            }else{
                $p++;
            ...
            $params[1] = $p; //修变分页值
            $pagtion['n'] = call_user_func_array('sprintf', $params);
