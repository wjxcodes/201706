...
<div class="operate">
    ...
    <!-- 查询区域 -->
    <FORM id="form1" METHOD="POST" ACTION="/Home/LogSearch">
    <div class="fRig">
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <TR>
            ...
            <TD><INPUT TYPE="text" NAME="UserName" class="medium" value="<?php echo ($_REQUEST['UserName']); ?>" ></TD>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["SearchID"]); ?>"></td>
        <td><?php echo ($node["SearchID"]); ?></td>
        <td><?php echo ($node["SubjectName"]); ?></td>
        <td><?php echo ($node["UserName"]); ?></td>
        <td><div style="max-width:480px;overflow:auto;"><?php echo ($node["KeyWord"]); ?></div></td>
        <td><?php echo ($node["Nums"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["LastTime"])); ?></td>
        <td><a href="<?php echo U('Home/LogSearch/showMsg',array('id'=>$node[SearchID]));?>" class="showmsg" thisid="<?php echo ($node["SearchID"]); ?>">查看详情</a>&nbsp;</td>
