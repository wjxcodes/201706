...
<div class="operate">
    ...
    <!-- 查询区域 -->
    <form id="form1" method="POST" action="/Manage/LogSms">
    <div class="fRig">
        <div class="fLeft">
            <span id="key">
                <input id="name" type="text" name="Receive" value="<?php echo ($_REQUEST['ReceiveName']); ?>" title="接收方查询" class="medium" >
            </span>
        </div>
    ...
    <div id="searchM" class=" none search cBoth">
        ...
        <tr>
            <td class="tRight" width="60">接收方：</td>
            <td>
                <input type="text" name="Receive" class="medium" value="<?php echo ($_REQUEST['Receive']); ?>" >
            </td>
            <td class="tRight" width="50">状态：</td>
            <td>
                <select class="medium bLeft" name="Status">
                    <option value="">全部</option>
                    <option value="0" <?php if(($_REQUEST['Status']) == "0"): ?>selected="selected"<?php endif; ?>>发送成功</option>
                    <option value="1" <?php if(($_REQUEST['Status']) == "1"): ?>selected="selected"<?php endif; ?>>发送失败</option>
                </select>
            </td>
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["SmsID"]); ?>"></td>
        <td><?php echo ($node["SmsID"]); ?></td>
        <td><?php echo ($node["UserName"]); ?></td>
        <td><?php echo ($node["CodeNum"]); ?></td>
        <td><?php echo ($node["Receive"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$node["AddTime"])); ?></td>
        <td><?php if(($node["Status"]) == "0"): ?>是<?php else: ?>否<?php endif; ?></td>
        <td><?php if(($node["IfConfirm"]) == "0"): ?>否<?php else: ?>是<?php endif; ?></td>
