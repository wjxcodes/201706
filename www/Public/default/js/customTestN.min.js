...
$.CustomTestStore={url:'',requestMark:false,knowledge:0,chapter:0,grade:'',types:'',subjectID:Cookie.Get('SubjectId'),username:Cookie.Get('wln_user_USER'),act:'',host:'',isFormat:false,showDialog:function(elemnt,title,delay,msg,type,YesHandler,NoHandler){var btn=[];btn.push('确定');if(type==2){btn.push('取消');}
layer.open({type:1,title:title,area:['300px','250px'],btn:btn,content:'<div style="padding:10px; ">'+msg+'</div>',yes:YesHandler||function(){layer.closeAll();},cancel:NoHandler||function(){layer.closeAll();}});},init:function(act,data,ifImage){this.act=act;this.InitDivHeight();if(!ifImage){$.Editor.init(U($.CustomTestStore.url+'/upload?dir=customTest'));FormatTextManager.types=$('#types');FormatTextManager.init();$('#format').click(function(){$.CustomTestStore.isFormat=false;if(FormatTextManager.formatContent()){$.CustomTestStore.showDialog('msgdiv','错误提示',500,FormatTextManager.err,1);return false;}
$.CustomTestStore.isFormat=true;$.CustomTestStore.showDialog('msgdiv','提示',500,'试题格式化完成！请重新检查内容，确认无误后点击[保存]！',1);return false;});$('#save').click(function(){if($.Editor.requestMark){$.CustomTestStore.showDialog('msgdiv','提示',500,'内容已提交，正在处理中，请稍候！',1);return false;}
var data=$.CustomTestStore.validate();if(data){$.Editor.requestMark=true;$.post(U($.CustomTestStore.url+'/save'),data,function(result){$.Editor.requestMark=false;var msg=result['data'];msg+='<br>点击 <strong>确定</strong> 再添一题，点击 <strong>取消</strong> 跳转校本试题！</a>';$.CustomTestStore.showDialog('msgdiv','提示',500,msg,2,function(){window.location.href=U('Custom/CustomTestStoreN/add');},function(){window.location.href=U('Custom/CustomTestStoreN/index');});});}
return false;});}
...
this.editing(data);}
$('.toggleAttributes').click(function(){var that=$(this);$(this).parents('tr').nextAll('tr').toggle();if(that.html()=='隐藏'){that.html('展开');}else{that.html('隐藏');}});this.setBasicData();this.showTip('您当前正在编辑协同命制试题！');},showTip:function(message){if(Cookie.Get('user_ORIGINALITY')){var tip='<div>'+'<span style="color: #333333" >'+(message?message:'您当前正在编辑协同命制试题！')+'</span>'+'<a id="goIndexOriginality" style="text-decoration: underline;cursor: pointer">返回协同命制平台</a> '+'<a id="quitOriginality" style="color: #ff0000;text-decoration: underline;cursor: pointer">退出编辑</a>'+'</div>';$(tip).Tips({'delay':1000*7200});}
$(document).on('click','#goIndexOriginality',function(){top.window.location.href=U('Index/Special/orginality?SubjectID='+Cookie.Get('SubjectId'));Cookie.Del('user_ORIGINALITY');});$(document).on('click','#quitOriginality',function(){window.location.href=U('Home/Index/content');Cookie.Del('user_ORIGINALITY');});},editing:function(data){var basic=data['basic'];if(basic['SubjectID']!=$.CustomTestStore.subjectID){alert('所编辑的试题与当前学科不符！');window.location.href=$.CustomTestStore.url;return false;}
...
param.push(str);}
return param.join('&');},addData:function(data,params){if(!data)
...
return str+=('>'+text+'</option>');},setTestNum:function(num){},validate:function(){var data={};data['act']=this.act;data['verifyCode']=$('#verifyCode').val();data['TTID']=$('#ttid').val();data['TestID']=$('#testid').val();if(!$.CustomTestStore.username){$.CustomTestStore.showDialog('msgdiv','错误提示',500,'用户名不能为空',1);return false;}
...
data['Source']=source;var type=$('#types').val();if(!type){type=types[$.CustomTestStore.subjectID][0]['TypesID'];}
data['TypesID']=type;var diff=$("input[name='diff']:checked").val();if(!diff){diff=0.801;}
...
$.CustomTestStore.showDialog('msgdiv','错误提示',500,error.join('，'),1);errors[0]['editor'].focus();return false;}
...
var num=$(this).attr('num');self.imageCutting(self.picData,num);$('#imgBtn'+num).removeClass('imgCut');$('#imgBtn'+num).addClass('saveImgCut');$('#imgBtn'+num).html('保存');});$(document).on('click','.saveImgCut',function(){var num=$(this).attr('num');self.avatarSave(num);$('#imgBtn'+num).removeClass('saveImgCut');$('#imgBtn'+num).addClass('imgCut');$('#imgBtn'+num).html('裁切');});$('#saveImgTest').on('click',function(){var data=[];$('.imgTest').each(function(){var picPath=$(this).find('#prevImg').attr('src');var testText=$(this).find('.testText').val();var test={'picPath':picPath,'testText':testText};data.push(test);});if(data.length==0){$.CustomTestStore.showDialog('msgdiv','提示',500,'<div>请先上传图片编辑！</div>',1);return false;}
$.post(U('Custom/CustomTestStoreN/imgTestSave'),{'data':data,'attr':self.getTestAttr()},function(e){if(e.status==1){if($.myCommon.backLogin(e)==false){return false;}
var msg=e['data'];msg+='<br>点击 <strong>确定</strong> 再添一题，点击 <strong>取消</strong> 跳转校本试题！</a>'
$.CustomTestStore.showDialog('msgdiv','提示',500,msg,2,function(){window.location.href=U('Custom/CustomTestStoreN/photograph');},function(){window.location.href=U('Custom/CustomTestStoreN/index');});}else{var alertHtml='<div>'+e.data+' 请刷新本页面后重试！</div>';$.CustomTestStore.showDialog('msgdiv','提示',500,alertHtml,1);}})
return false;});$(document).on('click','.del-btn',function(){var num=$(this).attr('num');$('#originImg'+num).imgAreaSelect({hide:true});$('#imgTest'+num).remove();var len=$('.imgTest').length;if(len<1){$('.submitBtn').hide();}});},init:function(username){this.getQrCode(username);this.avatarUpload();this.bindImg();}};$.customTestStoreTestAdd={init:function(data,originality,url,ifImage,act){if(url.indexOf('/')===0){url=url.substring(1);}
this.pageInit(data,originality,url,ifImage,act);},pageInit:function(data,originality,url,ifImage,act){$.CustomTestStore.url=url;if(!originality['ttID']&&'add'==act){$.CustomTestStore.init('add',data,ifImage);}else{if(originality['ttID']){$('#ttid').val(originality['ttID']);data.basic={};data.basic.TypesID=originality['typeID'];data.basic.SubjectID=Cookie.Get('SubjectId');data.basic.Diff=originality['diff'];var params={'ID':originality['klID'],'style':'knowledgeList','return':1};if(originality['klID']){$.post(U('Custom/CustomTestStoreN/getData'),params,function(result){data.knowledge=result['data'];$.CustomTestStore.init('originality',data,ifImage);});}}else{$.CustomTestStore.init('edit',data,ifImage);}}
$('.knowledge-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'KlID',vv:'KlName',identify:'knowledge',loadArea:'.knowledge-select-change-container-inputs'});$('.chapter-select-change-container').SelectChangeHandle({classname:'.selection',subjectid:$.CustomTestStore.subjectID,url:U(url+'/getData'),kv:'ChapterID',vv:'ChapterName',identify:'chapter',allowEmptyVal:true,loadArea:'.chapter-select-change-container-inputs'});}};$.fn.SelectChangeHandle=function(option){return $(this).each(function(){var opts=$.extend({loadBtn:'.append',classname:'select',loadArea:'.inputs',subjectid:false,allowEmptyVal:false,kv:'',vv:'',identify:'',url:'',callback:function(result,currentSel,selects){if($.myCommon.backLogin(result)==false){return false;};var datas=result['data'];finished=true;if(!datas){return false;}
...
var data={};data['subjectID']=opts.subjectid;data['pID']=_select.val();data['style']=opts.identify;sendRequest(_select,data);});that.find(opts.loadBtn).click(function(){var text=[];var vals=[];var selects=getSelects();var size=selects.length;for(var i=0;i<size;i++){var _select=$(selects[i]);if(_select.val()==''){if(!opts.allowEmptyVal||i<2){alert('请正确选择内容');_select.focus();return false;}}else{var selected=_select.find('option:checked');text.push(selected.text());vals.push(selected.val());}}
if(text.length==0){return false;}
vals=vals[vals.length-1];if(inputIsExist(vals)){var div=$('<div></div>');$(opts.loadArea).append(div.SelectText({_text:text.join('&nbsp;&gt;&nbsp;'),_name:opts.kv,val:vals}));}});function inputIsExist(val){var inputs=$(opts.loadArea).find('input');for(var i=0;i<inputs.length;i++){if(val==inputs[i].value){return false;}}
return true;}
