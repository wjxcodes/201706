...
<!-- 主体内容  -->
<div class="content" >
<div class="title"><?php echo ($pageName); ?> <a href="<?php echo U('Doc/DocSource/updateCache');?>">更新缓存</a></div>
<!--  功能操作区域  -->
<div class="operate">
...
<table id="checkList" class="list" cellpadding="5" cellspacing="0" border="1">
    ...
    <?php if(is_array($typeArray)): $i = 0; $__LIST__ = $typeArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="row lists" jl=''>
        <td><input type="checkbox" class="key" value="<?php echo ($node["SourceID"]); ?>"></td>
        <td><?php echo ($node["SourceID"]); ?></td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["SourceID"]); ?>"><?php echo ($node["SourceName"]); ?></a></td>
        <td><?php if($node["IfShowLogo"] == '1'): ?>显示<?php else: ?><font color="#f00">不显示</font><?php endif; ?></td>
        <td><?php echo ($node["LogoPath"]); ?></td>
        <td><?php if($node["IfFree"] == '1'): ?>免费<?php else: ?><font color="#f00">收费</font><?php endif; ?></td>
        <td><?php if($node["ValidityTime"] == '0'): ?>0<?php else: echo (date("Y-m-d H:i:s",$node["ValidityTime"])); endif; ?></td>
        <td><?php if($node['IfDefault'] == 1): ?>默认<?php else: ?>不默认<?php endif; ?></td>
        </td>
        <td><a href="#" class="btedit" thisid="<?php echo ($node["SourceID"]); ?>">编辑</a>&nbsp;</td>
